...
class PMS_Model():
    ...
    def rollout(self, s0, policy, T, dt, noise, vel_indeces, pos_indeces):
        ...
        num_samples = len(time)
        # get input size
        num_inputs = np.array(policy(s0, 0.)).size
        # allocate the space
        inputs = np.zeros([num_samples, num_inputs])
        states = np.zeros([num_samples, state_dim])
        noisy_states = np.zeros([num_samples, state_dim])
        ...
        for i, t in enumerate(time[:-1]):
            ...
            meas_states[i+1,vel_indeces] = (b[0]*noisy_states[i+1,vel_indeces] + b[1]*noisy_states[i,vel_indeces]- a[1]*meas_states[i,vel_indeces])/a[0]
