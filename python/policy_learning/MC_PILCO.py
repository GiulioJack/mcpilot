...
class MC_PILCO(torch.nn.Module):
    ...
    def __init__(self, T_sampling, state_dim, input_dim, f_sim,
                 ...
                 init_policy_dict=None, flg_save_all_particles_to_log=False):
        ...
        if init_policy_dict is not None:
            print('\n\n\n\n----------------- Load init policy parameters -----------------')
            self.control_policy.load_state_dict(init_policy_dict)
        ...
        self.flg_save_all_particles_to_log = flg_save_all_particles_to_log
    ...
    def reinforce(self, initial_state, initial_state_var,
                  ...
                  flg_init_incr_uniform=False, init_incr_uniform_params=None,
        ...
        """
        # check if simulation-time == T_control
        if T_control_sim is None:
            T_control_sim = T_control
        # get initial data
        if not loaded_model:
            ...
            for expl_index in range(0, num_explorations):
                ...
                if flg_init_func:
                    ...
                    if random_initial_state:  # initial state randomly sampled
                        ...
                        elif flg_init_incr_uniform:
                            eps = init_incr_uniform_params['eps']
                            x0 = np.random.uniform(eps * init_low_bound, eps * init_up_bound)
            ...
            t_model_learning_list = []
            t_policy_learning_list = []
            num_gp_inputs_list = []
            opt_step_list = []
            ...
            t_model_learning_list = self.log_dict['t_model_learning_list']
            t_policy_learning_list = self.log_dict['t_policy_learning_list']
            num_gp_inputs_list = self.log_dict['num_gp_inputs_list']
            opt_step_list = self.log_dict['opt_step_list']
        ...
        for trial_index in range(first_trial_index, last_trial_index):
            ...
            t_model_learning_list.append(t_model_learning_stop - t_model_learning_start)
            print('\nModel optimization time: ' + str(t_model_learning_stop - t_model_learning_start))
            num_gp_inputs_list.append(self.model_learning.num_gp_inputs_list.copy())

            with torch.no_grad():
                if self.log_path is not None:
                    print('\nSave log file...')
                    self.log_dict['parameters_gp_' + str(trial_index)] = [
                        copy.deepcopy(self.model_learning.gp_list[k].state_dict()) for k in
                        range(0, self.model_learning.num_gp)]
                    ...
                    self.log_dict['t_model_learning_list'] = t_model_learning_list
                    self.log_dict['num_gp_inputs_list'] = num_gp_inputs_list
                ...
                _, _, _, _ = self.get_model_learning_performance(data_collection_index=trial_index, post_str='post_tr')
            ...
            t_policy_learning_start = time.time()
            cost_list, std_cost_list, particles_states, particles_inputs, opt_step_all = \
                self.reinforce_policy(particles_initial_distribution_dict=particles_initial_distribution_dict,
                                      T_control=T_control_sim,
                                      trial_index=trial_index,
                                      **policy_optimization_dict)
            t_policy_learning_stop = time.time()
            t_policy_learning_list.append(t_policy_learning_stop - t_policy_learning_start)
            print('\nPolicy optimization time: ' + str(t_policy_learning_stop - t_policy_learning_start))
            print('Average iteration time: ' + str((t_policy_learning_stop - t_policy_learning_start) / opt_step_all))
            ...
            opt_step_list.append(opt_step_all)
            ...
            if self.log_path is not None:
                ...
                self.log_dict['t_policy_learning_list'] = t_policy_learning_list
                self.log_dict['opt_step_list'] = opt_step_list
                pkl.dump(self.log_dict, open(self.log_path + '/log.pkl', 'wb'))
            ...
            if flg_init_func:
                ...
                if random_initial_state:
                    if flg_init_uniform:
                        ...
                        eps = init_incr_uniform_params['eps']
                        num_initial = init_incr_uniform_params['num_initial']
                        num_increment = init_incr_uniform_params['num_increment']
                        x0 = np.random.uniform(init_low_bound * min(max(trial_index-num_initial, 0) / num_increment + eps, 1),
                                               init_up_bound  * min(max(trial_index-num_initial, 0) / num_increment + eps, 1))
                        # [-eps * lb, eps * ub] for trial up to num_initial
                        # then it goes to [lb, ub] incrementally in num_increment trials
                    elif flg_init_multi_gauss:
                        num_init = np.random.randint(initial_state.shape[0])
                        x0 = np.random.normal(initial_state[num_init, :], np.sqrt(initial_state_var[num_init, :]))
                    else:
                        x0 = np.random.normal(initial_state, np.sqrt(initial_state_var))
    ...
    def get_model_learning_performance(self, data_collection_index, flg_pretrain=False, post_str=''):
        """
        Test model learning performance in the data_collection_index simulation
        """
        # get gp predictions
        gp_inputs, gp_outputs_target_list, \
            gp_output_mean_list, gp_output_var_list = self.model_learning.get_gp_estimate_from_data(
            states=torch.tensor(self.state_samples_history[data_collection_index],
        ...
        gp_outputs_target_list = [gp_outputs_target_list[i].detach().cpu().numpy()
                                  for i in range(len(gp_outputs_target_list))]
        gp_output_mean_list = [gp_output_mean_list[i].detach().cpu().numpy()
                               for i in range(len(gp_output_mean_list))]
        gp_output_var_list = [gp_output_var_list[i].detach().cpu().numpy()
                              for i in range(len(gp_output_var_list))]
        # get gp performance
        print('len(gp_outputs_target_list)', len(gp_outputs_target_list))
        for gp_index in range(len(gp_outputs_target_list)):
            ...
            plt.figure()
            plt.title('Estimates GP ' + str(gp_index))
            plt.plot(gp_outputs_target_list[gp_index], label='y ' + str(gp_index), color='blue')
            plt.plot(gp_output_mean_list[gp_index], label='y ' + str(gp_index) + ' hat', color='red')
            std = np.sqrt(gp_output_var_list[gp_index])
            x = np.arange(gp_output_mean_list[gp_index].size)
            plt.fill(np.concatenate([x, np.flip(x)]),
                     np.concatenate([(gp_output_mean_list[gp_index] + 3 * std).reshape([-1]),
                                     np.flip((gp_output_mean_list[gp_index] - 3 * std).reshape([-1]))]), color='red',
                     alpha=0.2)
            plt.grid()
            plt.legend()
            plt.savefig(self.log_path + '/gp' + str(gp_index) + '_trial' + str(data_collection_index) + '_' +
                        post_str + '.pdf')
            plt.close()
        # plt.show()
    ...
    def test_model_learning(self, initial_state, initial_state_var, T_exploration, num_trials, #num_simulation_samples,
                            model_optimization_opt_list, num_explorations=1, flg_init_uniform=False, init_up_bound=None,
                            init_low_bound=None, flg_init_multi_gauss=False, flg_init_func=False, init_func=None,
                            random_initial_state=True, plot_noised_trs=False, verbose_print=False, vel_indices=None):
        """
        Model learning test
        """
        print('\n\n\n\n----------------- INITIAL EXPLORATIONS -----------------')
        ...
        for expl_index in range(0, num_explorations):
            ...
            self.get_data_from_system(initial_state=x0,
                                      ...
                                      flg_exploration=True,  # exploration interaction
                                      trial_index=expl_index)
            print('Exp samples: {}'.format(self.state_samples_history[0].shape))
    ...
    def plot_model_learning(self, trials_results, trial_index, actual_trajectory, rollout_states,
    ...
    def plot_GP_inference(self, gp_outputs_target_list, gp_output_mean_list, vel=None, title=''):
        for gp_index in range(0, self.model_learning.num_gp):
            if vel is None:
                plt.figure()
                plt.title(('GP {} predictions '+title).format(gp_index + 1))
                plt.plot(gp_outputs_target_list[gp_index], color='r', label='ref', linestyle='--')
                plt.plot(gp_output_mean_list[gp_index], label='gp', color='b')
                plt.legend()
                plt.grid()
            else:
                plt.figure()
                ...
                plt.grid()
                plt.subplot(2,1,2)
                plt.plot(vel[:,gp_index], label='vel')
                plt.plot(gp_outputs_target_list[gp_index]-gp_output_mean_list[gp_index], label='error')
                plt.legend()
                plt.grid()
    ...
    def get_rollout_prediction_performance(self, data_collection_index, T_rollout=None, add_name='',
        ...
        if self.log_path is not None and save_to_log:
            ...
            for state_dim_index in range(self.state_dim):
                ...
                plt.plot(self.state_samples_history[data_collection_index][:, state_dim_index],
                         label='true rollout ' + str(state_dim_index))
                plt.grid()
                plt.legend()
                plt.grid()
                plt.savefig(self.log_path + '/dim' + str(state_dim_index) + '_trial' + str(
                    data_collection_index) + '_' + add_name + '.pdf')
                plt.close()
            plt.show()
    ...
    def rollout(self, data_collection_index, T_rollout=None, particle_pred=False):
        ...
        for t in range(1, T_rollout):
            ...
            rollout_trj[t:t + 1, :], _, _ = self.model_learning.get_next_state(current_state=rollout_trj[t - 1:t, :],
                                                                               current_input=inputs_trajectory_tc[
                                                                                             t - 1:t, :],
    ...
    def reinforce_policy(self, T_control, num_particles, trial_index,
                         ...
                         alpha_cost=0.99, alpha_input=0.99, alpha_diff_cost=0.99, lr_reduction_ratio=0.5, lr_min=0.001,
                         p_drop_reduction=0., num_step_save_policy=25,
        ...
        # set particles initial distribution
        self.set_particles_initial_distribution(particles_initial_distribution_dict, num_particles, trial_index=trial_index)
        ...
        with torch.no_grad():
            ...
            while num_attempts < 10 and flg_nan:
                ...
                states_sequence_NODROP, inputs_sequence_NODROP = self.apply_policy(num_particles=num_particles,
                                                                                   T_control=control_horizon,
                                                                                   p_dropout=p_dropout_applied)
                # initial cost with no dropout applied
                cost_NODROP, std_cost_NODROP = self.cost_function(states_sequence_NODROP, inputs_sequence_NODROP,
                                                                  trial_index)
        ...
        while opt_step < num_opt_steps:
            ...
            while num_attempts < 10 and flg_nan:
                # apply the policy
                states_sequence, inputs_sequence = self.apply_policy(num_particles=num_particles,
                                                                     T_control=control_horizon,
                                                                     p_dropout=p_dropout_applied)
            ...
            cost_list[opt_step] = cost.data.detach()
            std_cost_list[opt_step] = std_cost.data.detach()
            ...
            if num_step_save_policy > 0 and opt_step % num_step_save_policy == 0:
                model_dict = self.control_policy.state_dict()
                if not 'control_policy' in self.log_dict.keys():
                    self.log_dict['control_policy'] = {trial_index: [copy.deepcopy(model_dict)]}
                    self.log_dict['control_policy']['num_step_save_policy'] = num_step_save_policy
                else:
                    if trial_index in self.log_dict['control_policy'].keys():
                        # if len(self.log_dict['control_policy'][trial_index]) > 0:
                        self.log_dict['control_policy'][trial_index].append(copy.deepcopy(model_dict))
                    else:
                        self.log_dict['control_policy'][trial_index] = [copy.deepcopy(model_dict)]
            ...
            if opt_step > current_min_step:
                if torch.sum(torch.abs(diff_cost_ratio[opt_step+1-num_min_diff_cost:opt_step+1]) < current_min_diff_cost) >= num_min_diff_cost:
                    if lr > lr_min:
                        ...
                        print('lr: ', lr)
                        current_min_diff_cost = max(current_min_diff_cost / 2, 0.00001)
                        current_min_step = opt_step + num_min_diff_cost
                        optimizer = f_optim(p=self.control_policy.parameters(), lr=lr)
                        print('REDUCING THE DROPOUT:')
                        p_dropout_applied = max(p_dropout_applied - p_drop_reduction, 0.0)
                        print('p_dropout_applied: ', p_dropout_applied)

                        if self.log_path is not None:
                            if 'dropout_history' in self.log_dict.keys():
                                if trial_index in self.log_dict['dropout_history'].keys():
                                    self.log_dict['dropout_history'][trial_index].append([opt_step, p_dropout_applied])
                                else:
                                    self.log_dict['dropout_history'][trial_index] = [[opt_step, p_dropout_applied]]
                            else:
                                self.log_dict['dropout_history'] = {trial_index: [[opt_step, p_dropout_applied]]}
            ...
            if flg_nan:
                ...
                std_cost_list = torch.zeros(num_opt_steps, device=self.device, dtype=self.dtype)
                gradients_list = []
                states_sequence_std_list = torch.zeros([self.state_dim, num_opt_steps], device=self.device,
                                                       dtype=self.dtype)
                ...
                mean_states_list = torch.zeros([self.state_dim, num_opt_steps, control_horizon], device=self.device,
                                               dtype=self.dtype)
                mean_inputs_list = torch.zeros([self.input_dim, num_opt_steps, control_horizon], device=self.device,
                                               dtype=self.dtype)
                drop_list = []
                ES1_diff_cost = torch.zeros(num_opt_steps + 1, device=self.device, dtype=self.dtype)
                diff_cost_ratio = torch.zeros(num_opt_steps + 1, device=self.device, dtype=self.dtype)
            ...
            # Save last states of particles to log
            if self.flg_save_all_particles_to_log and not flg_nan:
                states_sequence_list.append(states_sequence[-1, :, :].detach().cpu().numpy())
                inputs_sequence_list.append(inputs_sequence[-1, :, :].detach().cpu().numpy())

        # Save only last batch of particles to log
        if not self.flg_save_all_particles_to_log:
            states_sequence_list = states_sequence.detach().cpu().numpy()
            inputs_sequence_list = inputs_sequence.detach().cpu().numpy()
        else:  # otherwise save full particle evolution of the last epoch
            states_sequence_list.append(states_sequence.detach().cpu().numpy())
            inputs_sequence_list.append(inputs_sequence.detach().cpu().numpy())
        ...
        return cost_list, std_cost_list, states_sequence_list, inputs_sequence_list, opt_step_all

    def apply_policy(self, num_particles, T_control, p_dropout=0.):
        """
        ...
        states_sequence_list.append(self.initial_particles_distribution.rsample())

        # compute initial inputs
        inputs_sequence_list.append(self.control_policy(states_sequence_list[0], t=0, p_dropout=p_dropout))

        # check flg_parametric_model
        if self.model_learning.flg_parametric_model:
            with torch.no_grad():
                # sample parameters (one parameters vector for each particle)
                W_list = self.model_learning.sample_model_parameters(num_particles)
            particle_pred = False
        else:
            particle_pred = getattr(self, 'sample_gp_posterior', True)

        for t in range(1, int(T_control)):

            # get next state mean and variance (given the states sampled and the inputs computed)
            if self.model_learning.flg_parametric_model:
                particles, _, _ = self.model_learning.get_next_state_parametric_model(
    ...
    def load_model_from_log(self, num_trial, folder='results_tmp/1/'):
        ...
        self.model_learning.init_gp_models()
        for k in range(self.model_learning.num_gp):
            print(self.model_learning.gp_list[k].state_dict().keys())
            print(log_dict['parameters_gp_' + str(trial_index)][k].keys())
            if self.model_learning.approximation_mode == 'SOR':
                self.model_learning.gp_list[k].init_inducing_inputs(self.model_learning.gp_inputs[::self.model_learning.approximation_dict['downsampling_rate'],:],
                                                                    flg_train_inducing_inputs=self.model_learning.approximation_dict['flg_regressors_trainable'],)
            self.model_learning.gp_list[k].load_state_dict(log_dict['parameters_gp_' + str(trial_index)][k])

        # Rebuild posterior caches as constants.  These caches are reused by
        # every policy-gradient step and must not retain a one-shot autograd
        # graph from model loading.
        with torch.no_grad():
            for k in range(self.model_learning.num_gp):
                self.model_learning.pretrain_gp(k)

    def load_model_snapshot_from_log(self, data_collection_index,
                                     num_data_collections,
                                     folder='results_tmp/1/'):
        """Load one frozen GP snapshot and an exact prefix of its source data.

        Unlike ``load_model_from_log``, this method separates the GP snapshot
        index from the number of trajectories restored.  This is needed for a
        policy-only rerun: snapshot 4 was trained on exploration trajectories
        0..4, while trajectory 5 is the later on-system policy rollout and must
        not be added to the experiment.
        """
        log_file_path = folder.rstrip('/') + '/log.pkl'
        print('\nLoading frozen GP snapshot from: ' + log_file_path)

        source_log = pkl.load(open(log_file_path, 'rb'))
        data_collection_index = int(data_collection_index)
        num_data_collections = int(num_data_collections)

        if data_collection_index < 0 or num_data_collections <= 0:
            raise ValueError('GP snapshot and trajectory count must be positive')
        if data_collection_index >= num_data_collections:
            raise ValueError(
                'GP snapshot {} cannot precede only {} restored trajectories'.format(
                    data_collection_index, num_data_collections
                )
            )

        required_snapshot_keys = [
            'parameters_gp_{}'.format(data_collection_index),
            'gp_inputs_{}'.format(data_collection_index),
            'gp_output_list_{}'.format(data_collection_index),
        ]
        for key in required_snapshot_keys:
            if key not in source_log:
                raise KeyError('Missing {} in {}'.format(key, log_file_path))

        for history_key in ('state_samples_history', 'input_samples_history'):
            if len(source_log.get(history_key, [])) < num_data_collections:
                raise ValueError(
                    '{} contains fewer than {} trajectories'.format(
                        history_key, num_data_collections
                    )
                )

        for j in range(num_data_collections):
            print('\nRestore exploration trajectory: {}/{}'.format(
                j, num_data_collections - 1
            ))
            state_samples = source_log['state_samples_history'][j]
            input_samples = source_log['input_samples_history'][j]
            self.state_samples_history.append(state_samples)
            self.input_samples_history.append(input_samples)
            if len(source_log.get('noiseless_states_history', [])) > j:
                self.noiseless_states_history.append(
                    source_log['noiseless_states_history'][j]
                )
            self.num_data_collection += 1
        ...
        snapshot_suffix = str(data_collection_index)
        self.model_learning.gp_inputs = source_log[
            'gp_inputs_' + snapshot_suffix
        ]
        self.model_learning.gp_output_list = source_log[
            'gp_output_list_' + snapshot_suffix
        ]
        self.model_learning.init_gp_models()

        snapshot_parameters = source_log[
            'parameters_gp_' + snapshot_suffix
        ]
        for k in range(self.model_learning.num_gp):
            if self.model_learning.approximation_mode == 'SOR':
                self.model_learning.gp_list[k].init_inducing_inputs(
                    self.model_learning.gp_inputs[
                        ::self.model_learning.approximation_dict[
                            'downsampling_rate'
                        ],
                        :,
                    ],
                    flg_train_inducing_inputs=self.model_learning.approximation_dict[
                        'flg_regressors_trainable'
                    ],
                )
            self.model_learning.gp_list[k].load_state_dict(
                snapshot_parameters[k]
            )

        # Recompute only posterior caches (alpha/K^-1); no GP optimization is
        # performed and no GP parameter is changed.
        with torch.no_grad():
            for k in range(self.model_learning.num_gp):
                self.model_learning.pretrain_gp(k)

        print(
            'Loaded GP snapshot {} with exactly {} trajectories'.format(
                data_collection_index, num_data_collections
            )
        )

    def set_particles_initial_distribution(self, particles_initial_distribution_dict, num_particles, trial_index=-1):
        # set particles initial state distribution
        if 'initial_distribution' not in particles_initial_distribution_dict.keys():
            raise ValueError('No initial_distribution defined')

        if particles_initial_distribution_dict['initial_distribution'] == 'uniform':
            # initial uniform distribution
            uniform_ub_particles = particles_initial_distribution_dict['particles_init_up_bound'].repeat(num_particles,1)
            uniform_lb_particles = particles_initial_distribution_dict['particles_init_low_bound'].repeat(num_particles,1)
            self.initial_particles_distribution = Uniform(uniform_lb_particles, uniform_ub_particles)
        elif particles_initial_distribution_dict['initial_distribution'] == 'incr_uniform':
            uniform_ub_particles = particles_initial_distribution_dict['particles_init_up_bound'].repeat(num_particles,1)
            uniform_lb_particles = particles_initial_distribution_dict['particles_init_low_bound'].repeat(num_particles,1)
            eps = particles_initial_distribution_dict['eps']
            num_initial = particles_initial_distribution_dict['num_initial']
            num_increment = particles_initial_distribution_dict['num_increment']
            gamma_k = np.clip((trial_index - num_initial) / num_increment, eps, 1)
            uniform_ub_particles *= gamma_k
            uniform_lb_particles *= gamma_k

            #print("uniform_lb_particles", uniform_lb_particles)
            #print("uniform_ub_particles", uniform_ub_particles)
            self.initial_particles_distribution = Uniform(uniform_lb_particles, uniform_ub_particles)
        elif particles_initial_distribution_dict['initial_distribution'] == 'multimodal-gaussian':
            # initial multimodal Gaussian distribution
            indices = torch.randint(0,particles_initial_distribution_dict['particles_initial_state_mean'].shape[0], [num_particles])
            initial_particles_mean = particles_initial_distribution_dict['particles_initial_state_mean'][indices,:]
            initial_particles_cov_mat = torch.stack([torch.diag(particles_initial_distribution_dict['particles_initial_state_var'][i,:]) for i in indices])
            self.initial_particles_distribution = MultivariateNormal(loc=initial_particles_mean, covariance_matrix=initial_particles_cov_mat)
        elif particles_initial_distribution_dict['initial_distribution'] == 'gaussian': 
            # initial Gaussian distribution
            initial_particles_mean = particles_initial_distribution_dict['particles_initial_state_mean'].repeat(num_particles,1)
            initial_particles_cov_mat = torch.stack([torch.diag(particles_initial_distribution_dict['particles_initial_state_var'])]*num_particles)
            self.initial_particles_distribution = MultivariateNormal(loc = initial_particles_mean, covariance_matrix = initial_particles_cov_mat)
        elif particles_initial_distribution_dict['initial_distribution'] == 'multimodal-gaussian-from-data': 
            initial_states = torch.tensor([self.state_samples_history[i][0, :] for i in range(len(self.state_samples_history))],
                                          dtype=self.dtype, device=self.device)

            initial_particles_mean = np.mean(initial_states, axis=0).repeat(num_particles,1)
            initial_particles_cov_mat = torch.stack([torch.diag(np.var(initial_states, axis=0))]*num_particles)
            self.initial_particles_distribution = MultivariateNormal(loc=initial_particles_mean, covariance_matrix=initial_particles_cov_mat)
        elif particles_initial_distribution_dict['initial_distribution'] == 'multiple_gaussians':
            # initial Gaussians distributions
            num_distributions = particles_initial_distribution_dict['num_distributions']
            initial_particles_mean = particles_initial_distribution_dict['particles_initial_state_mean'][0].repeat(int(num_particles/num_distributions), 1)
            initial_particles_cov_mat = [torch.diag(particles_initial_distribution_dict['particles_initial_state_var'][0])] * int(num_particles/num_distributions)
            for k in range(num_distributions-1):
                initial_particles_mean = torch.concatenate([initial_particles_mean, particles_initial_distribution_dict['particles_initial_state_mean'][k+1].repeat(int(num_particles/num_distributions), 1)], dim=0)
                initial_particles_cov_mat = initial_particles_cov_mat + [torch.diag(particles_initial_distribution_dict['particles_initial_state_var'][k+1])] * int(num_particles/num_distributions)
            initial_particles_cov_mat = torch.stack(initial_particles_cov_mat)
            # print(initial_particles_mean.size())
            # print(initial_particles_cov_mat.size())
            self.initial_particles_distribution = MultivariateNormal(loc = initial_particles_mean, covariance_matrix = initial_particles_cov_mat)
        elif particles_initial_distribution_dict['initial_distribution'] == 'custom_function':
            initial_states = torch.zeros((num_particles, self.state_dim))
            self.initial_particles_distribution = particles_initial_distribution_dict['init_func']
        else:
            raise NotImplementedError(particles_initial_distribution_dict['initial_distribution'])
...
class MC_PILCO4PMS(MC_PILCO):
    ...
    def apply_policy(self, num_particles, T_control, p_dropout):
        ...
        filter = False
        if 'fc' in self.filtering_dict.keys():
            filter = True
        ...
        for t in range(1, int(T_control)):
            ...
            particles, _, _ = self.model_learning.get_next_state(current_state=states_sequence_list[t - 1],
                                                                 current_input=inputs_sequence_list[t - 1],
                                                                 particle_pred=getattr(
                                                                     self, 'sample_gp_posterior', True
                                                                 ))
            states_sequence_list.append(particles)

            # get the noisy states (add noise to positions)
            noisy_state_samples = states_sequence_list[t].clone()
            noisy_state_samples[:, self.pos_indeces] = (noisy_state_samples[:, self.pos_indeces]
                                                        + std_pos_noise * torch.randn(num_particles,len(self.pos_indeces),
                                                                                                       device=self.device,
                                                                                                       dtype=self.dtype))
            ...
            if filter:
                meas_state_samples[:, self.vel_indeces] = (b[0] * noisy_states_sequence_list[t][:, self.vel_indeces]
                                                           + b[1] * noisy_states_sequence_list[t - 1][:, self.vel_indeces]
                                                           - a[1] * meas_states_sequence_list[t - 1][:, self.vel_indeces]) / a[0]
            meas_states_sequence_list.append(meas_state_samples)
    ...
    def get_data_from_system(self, initial_state, T_exploration, trial_index, flg_exploration=False):
        ...
        if flg_exploration:
            ...
            for name, param in self.control_policy.named_parameters():
                np.savetxt(self.log_path + '/DATA_' + str(trial_index-1)+'/policy_' + name + '.csv', param.data.cpu().numpy(), delimiter=',')
        ...
        while (not done):
            ...
            if cmd == 'q':
                ...
                try:
                    noisy_samples = np.genfromtxt(self.log_path + '/DATA_' + str(trial_index) + '/noisy_samples.csv',
                                                  delimiter=',')
                    input_samples = np.genfromtxt(self.log_path + '/DATA_' + str(trial_index) + '/input_samples.csv',
                                                  delimiter=',').reshape([-1, self.input_dim])
                except:
